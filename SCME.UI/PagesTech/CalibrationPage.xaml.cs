...
//                    NavigationService.Navigate(Cache.dVdt);
        ...
        private void Pulse_Igate_Click(object Sender, RoutedEventArgs E)
        {
            try
            {
        ...
        private void Pulse_Vgate_Click(object Sender, RoutedEventArgs E)
        {
            try
            {
                //var result = Cache.Net.GatePulseCalibrationGate(ushort.Parse(calibrationVoltage.Text));
                
                var result = Cache.Net.GatePulseCalibrationMain(ushort.Parse(calibrationVoltage.Text));

                actualVoltage.Content = result.ToString(CultureInfo.InvariantCulture);
                actualCurrent.Content = "0";
            }
            catch
            {
            }
