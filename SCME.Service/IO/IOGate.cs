...
namespace SCME.Service.IO
{
    internal class IOGate
    {
        ...
        internal Tuple<double, ushort> PulseCalibrationGate(ushort Current)
        {
            SystemHost.Journal.AppendLog(ComplexParts.Gate, LogMessageType.Info,
                                         string.Format("Calibrate Igate, current - {0}", Current));

            if (m_IsGateEmulation)
                return new Tuple<double, ushort>(0, 0);
            ...
            try
            {
                ...
                var resultCurrent = ReadRegister(REG_RES_CAL_CURRENT) + ReadRegister(233) / 1000.0;

                return new Tuple<double, ushort>(resultCurrent, 0);
            }
            finally
            {
        ...
        internal double PulseCalibrationMain(ushort Voltage)
        {
            SystemHost.Journal.AppendLog(ComplexParts.Gate, LogMessageType.Info,
                                         string.Format("Calibrate Vgate, voltage - {0}", Voltage));
            ...
            try
            {
                ...
                var resultVoltage = ReadRegister(REG_RES_CAL_CURRENT) + ReadRegister(233) / 1000.0;
